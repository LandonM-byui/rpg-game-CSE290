...
## Cache of an [IContext] class name: [Array] of [IContext] related to it from [IContext.related_context()]
static var related_icontext := {}
...
static func _cache_class_inheritance(cn: String, base: String) -> void:
	...
	if base == "IContext":
		var cls := load(class_cache[cn]).new() as IContext
		var rel_ctxt := cls.related_context()
		related_icontext[cn] = rel_ctxt